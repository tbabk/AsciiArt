...
class App(args: Array[String]) {
  ...
  private var ascii_converter : AsciiConverter = new AsciiConverter()
  private var gs_converter : GreyScaleConverter = new GreyScaleConverter()

  // TODO
  private var min_height = 4
  private var max_height = 100
  private var min_width = 4
  ...
  private def setCustomTable(custom: customTable) : Unit = {
    var non_linear_class_arr = ArrayBuffer[NonLinearClass]()

    for (rule <- custom.rules) {
      non_linear_class_arr += new NonLinearClass(rule.character, rule.start, rule.end)
    }
  ...
  private def filterImage(filters: List[inFilter]) : Unit = {
    ...
    var filter_applier : ascii.FilterApplier = new ascii.FilterApplier(grey_img)
    for(filter <- filters) {
